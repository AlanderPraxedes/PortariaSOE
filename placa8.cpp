...
cv::Rect largestPlate;
int largestPlateSize = 0;
...
for (const auto& plate : plates) {
    ...
    int plateSize = plate.width * plate.height;
    if (plateSize > largestPlateSize) {
        largestPlate = plate;
        largestPlateSize = plateSize;
    }
}

// Verificar se uma região foi encontrada
if (largestPlateSize > 0) {
    // Criar uma nova imagem contendo apenas a maior região selecionada
    cv::Mat plateImage = image(largestPlate).clone();
...
// Carregar a imagem da placa
   ...
   if (resultIterator != nullptr ) {
        do {
            ...
            if (symbol != nullptr && confidence > 80.00 ) {
